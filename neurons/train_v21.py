"""v21 — Hierarchical Action Transformer trained on benchmark + aux humans + pseudo.

v20 used a tiny 19K-param transformer that didn't move the needle. v21 scales:
- Action encoder: action_type + street + seat + amount_bucket embeddings
- Hand encoder: 2-layer self-attention over action sequence (64-dim, 4 heads)
- Chunk encoder: 2-layer self-attention over hand embeddings
- Total params ~150-250K (vs v20's 19K)

Training mix:
- Benchmark: 4200 labeled chunks (real ground truth) — primary signal
- Aux humans: 32,088 human hands -> ~800 synthetic 40-hand human chunks
  (matching top miner Travis's approach; gives us strong human-vs-bot baseline)
- Pseudo: 2937 pseudo-labeled live chunks (downweighted to 0.3)

Inspired by tomkaba/poker44-miner-ml13tens7 (current #1) and Travis861/Poker44_v1
(former #1) — both use sequence models + auxiliary human corpus.
"""
...
MODEL_OUTPUT = PROJECT / "neurons" / "model_v21_transformer.pkl"
...
def main():
    ...
    print("=" * 60)
    print("v21 Hierarchical Action Transformer")
    print("=" * 60)

    print("\n[1/4] Loading data...")
    bench_cks, bench_y = load_benchmark()
    aux_cks, aux_y = load_aux_humans(chunks_to_build=800)
    ...
    class HierarchicalActionTransformer(nn.Module):
        def __init__(self):
            super().__init__()
            self.action_emb = nn.Embedding(len(ACTION_TYPES), 24)
            self.street_emb = nn.Embedding(len(STREETS), 8)
            self.seat_emb = nn.Embedding(10, 8)
            self.amount_emb = nn.Embedding(len(AMOUNT_BUCKETS), 24)
            self.action_proj = nn.Linear(24 + 8 + 8 + 24, EMBED_DIM)
        ...
        def forward(self, x):
            ...
            # masking actions where action_type==empty(idx 9)
            action_mask = (a == len(ACTION_TYPES) - 1)
            e = self.hand_encoder(e, src_key_padding_mask=action_mask)
            hand_lengths = (~action_mask).sum(dim=1).clamp(min=1).unsqueeze(-1).float()
            hand_emb = (e * (~action_mask).unsqueeze(-1).float()).sum(dim=1) / hand_lengths
            hand_emb = hand_emb.view(B, H, -1) + self.hand_pos.unsqueeze(0)
            # masking hands where all actions empty (heuristic: first action's type==empty)
            hand_mask = (x[:, :, 0, 0] == len(ACTION_TYPES) - 1)
            chunk_e = self.chunk_encoder(hand_emb, src_key_padding_mask=hand_mask)
            chunk_lengths = (~hand_mask).sum(dim=1).clamp(min=1).unsqueeze(-1).float()
            chunk_emb = (chunk_e * (~hand_mask).unsqueeze(-1).float()).sum(dim=1) / chunk_lengths
            return self.head(chunk_emb).squeeze(-1)

    model = HierarchicalActionTransformer().to(device)
