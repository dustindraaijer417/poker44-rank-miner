"""v24 — Padding-leak-free transformer (gen4).

v23 collapsed (98% bot) because aux chunks were fixed-length (40 hands)
and live chunks are variable. Model used padding pattern as a feature.

Fix in v24:
- Aux chunks now have VARIABLE lengths (25-70 hands, uniform) matching
  live distribution
- 1500 aux chunks (was 800)
- Same dimensions as v22 (48x18) for proven CPU speed
- 50 epochs (was 15) — more training time per iteration
- Stronger dropout (0.2) for robustness
"""
...
MODEL_OUTPUT = PROJECT / "neurons" / "model_v24_transformer.pkl"
...
def load_aux_humans(chunks_to_build: int = 1500, seed: int = 42):
    """Build VARIABLE-LENGTH human chunks matching live distribution.

    Live chunks vary from 36 to 88 hands (mean ~60). Earlier versions used
    fixed 40-hand chunks which gave the model a padding-pattern signal —
    v23 went 98% bot because of this. Here we sample random lengths in
    [25, 70] uniformly to avoid that leak.
    """
    ...
    chunks = []
    idx = 0
    for _ in range(chunks_to_build):
        length = rng.randint(25, 70)  # match live distribution
        if idx + length > len(hands):
            break
        chunks.append(hands[idx:idx + length])
        idx += length
...
def main():
    ...
    print("=" * 60)
    print("v24 Padding-Leak-Free Transformer (gen4)")
    print("=" * 60)

    print("\n[1/4] Loading data...")
    bench_cks, bench_y = load_benchmark()
    aux_cks, aux_y = load_aux_humans(chunks_to_build=1500, seed=42)
