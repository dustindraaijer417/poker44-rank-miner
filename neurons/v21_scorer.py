"""v21 Hierarchical Action Transformer scorer.

Loads the trained v21 model (per-hand + per-chunk attention) and produces
bot probabilities. ~150K params. Designed as the primary scorer in the
v21 ensemble for the v1.5+ harder-batch regime.
"""
...
MODEL_PATH = Path(__file__).resolve().parent / "model_v21_transformer.pkl"
...
class V21Scorer:
    def __init__(self):
        ...
        if not MODEL_PATH.exists():
            raise FileNotFoundError(f"v21 transformer not found at {MODEL_PATH}")
        ...
        class HierarchicalActionTransformer(nn.Module):
            def __init__(self):
                super().__init__()
                self.action_emb = nn.Embedding(N_ACT, 24)
                self.street_emb = nn.Embedding(N_STREET, 8)
                self.seat_emb = nn.Embedding(10, 8)
                self.amount_emb = nn.Embedding(N_BUCKET, 24)
        ...
        self.model = HierarchicalActionTransformer()
