"""V_GPU_V3 scorer — load and score the GPU-trained transformer."""
...
MODEL_PATH = Path(__file__).resolve().parent / "model_gpu_v3.pkl"


def _bucket_amount(norm_bb, buckets):
...
class VGPUv3Scorer:
    def __init__(self):
        ...
        if not MODEL_PATH.exists():
            raise FileNotFoundError(f"v_gpu_v3 model not found at {MODEL_PATH}")
        with open(MODEL_PATH, "rb") as f:
            p = pickle.load(f)
        self.embed_dim = p["embed_dim"]
        self.n_heads = p["n_heads"]
        self.n_layers_hand = p["n_layers_hand"]
        self.n_layers_chunk = p["n_layers_chunk"]
        self.ffn_dim = p["ffn_dim"]
        self.action_types = p["action_types"]
        self.streets = p["streets"]
        self.amount_buckets = p["amount_buckets"]
        self.max_actions = p["max_actions_per_hand"]
        self.max_hands = p["max_hands_per_chunk"]
        self.val_ap = p.get("live_val_ap", 0.0)

        EMBED_DIM, N_HEADS = self.embed_dim, self.n_heads
        N_LAYERS_HAND, N_LAYERS_CHUNK, FFN_DIM = self.n_layers_hand, self.n_layers_chunk, self.ffn_dim
        N_ACT, N_STREET, N_BUCKET = len(self.action_types), len(self.streets), len(self.amount_buckets)
        MAX_A, MAX_H = self.max_actions, self.max_hands

        class HierarchicalTransformerV3(nn.Module):
            def __init__(self):
                super().__init__()
                self.action_emb = nn.Embedding(N_ACT, 48)
                self.street_emb = nn.Embedding(N_STREET, 16)
                self.seat_emb = nn.Embedding(10, 16)
                self.amount_emb = nn.Embedding(N_BUCKET, 48)
                self.action_proj = nn.Linear(48 + 16 + 16 + 48, EMBED_DIM)
                ...
                hand_layer = nn.TransformerEncoderLayer(
                    ...
                    dropout=0.3, batch_first=True, norm_first=True, activation="gelu",
                )
        ...
        self.model = HierarchicalTransformerV3()
